...
interface CanvasCommand {
  action: 'draw_circle' | 'draw_rectangle' | 'adjust_brightness' | 'add_text' | 'apply_sepia' | 'apply_grayscale' | 'apply_invert' | 'apply_sharpen';
  params: {
    x?: number;
    y?: number;
    radius?: number;
...
const CommandInput: React.FC<CommandInputProps> = ({
  ...
  const parseCommand = useCallback((input: string): CanvasCommand | null => {
    ...
    // Sepia filter patterns - MOVED OUTSIDE OF add_text condition
    if (lowercaseInput.includes('sepia') || lowercaseInput.includes('vintage')) {
      return {
        action: 'apply_sepia',
        params: {}
      };
    ...
    // Grayscale filter patterns
    if (lowercaseInput.includes('grayscale') || lowercaseInput.includes('black and white') || lowercaseInput.includes('b&w')) {
      return {
    ...
    // Invert colors patterns
    if (lowercaseInput.includes('invert') || lowercaseInput.includes('negative')) {
      return {
        action: 'apply_invert',
        params: {}
      };
    ...
    // Sharpen patterns
    if (lowercaseInput.includes('sharpen') || lowercaseInput.includes('enhance')) {
      const intensityMatch = lowercaseInput.match(/(\d+)/);
      return {
        action: 'apply_sharpen',
        params: {
      ...
      switch (cmd.action) {
        ...
        case 'apply_grayscale':
          ...
          for (let i = 0; i < grayPixels.length; i += 4) {
            const gray = grayPixels[i] * 0.299 + grayPixels[i + 1] * 0.587 + grayPixels[i + 2] * 0.114;
            grayPixels[i] = gray;
            grayPixels[i + 1] = gray;
            grayPixels[i + 2] = gray;
          }
        ...
        case 'apply_sharpen':
          ...
          const intensity = cmd.params.intensity || 5;
          const sharpenPixels = sharpenData.data;
          for (let i = 0; i < sharpenPixels.length; i += 4) {
            sharpenPixels[i] = Math.max(0, Math.min(255, sharpenPixels[i] * (1 + intensity / 50)));
            sharpenPixels[i + 1] = Math.max(0, Math.min(255, sharpenPixels[i + 1] * (1 + intensity / 50)));
            sharpenPixels[i + 2] = Math.max(0, Math.min(255, sharpenPixels[i + 2] * (1 + intensity / 50)));
          }
  ...
  const processCommand = useCallback(async () => {
    ...
    try {
      ...
      if (!parsedCommand) {
        onError('Could not understand the command. Try: "draw red circle", "brighten 20%", "apply sepia", or "add text Hello"');
        return;
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="relative">
          <input
            ...
            placeholder="e.g., draw red circle, brighten 20%, apply sepia, add text..."
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• Supported: draw shapes, adjust brightness, add text, apply filters</p>
        <p>• Colors: red, blue, green, yellow, purple, orange, pink, white, black</p>
        <p>• Filters: sepia, grayscale, invert, sharpen</p>
...
//       }
//     }
//     // Add to the existing parseCommand function in CommandInput.tsx

//     // Sepia filter patterns
//     if (lowercaseInput.includes('sepia') || lowercaseInput.includes('vintage')) {
//       return {
//         action: 'apply_sepia',
//         params: {}
...
//         case 'apply_sharpen':
//           // Simple sharpen using convolution (basic implementation)
//           const sharpenData = ctx.getImageData(0, 0, canvas.width, canvas.height);
//           // This would need a more sophisticated convolution implementation
//           // For now, we'll use a brightness/contrast approximation
//           const intensity = cmd.params.intensity || 5;
//           const sharpenPixels = sharpenData.data;
//           for (let i = 0; i < sharpenPixels.length; i += 4) {
