...
import { isModelInitialized, processImage } from '../utils/process';
...
const AIBackgroundRemoval: React.FC<AIBackgroundRemovalProps> = ({
  ...
  useEffect(() => {
    const checkModelInitialized = () => {
      if (isModelInitialized()) {
        setIsModelLoaded(true);
      } else {
        // Check again after a short delay if not initialized yet
        setTimeout(checkModelInitialized, 500);
      }
    };
  ...
  const removeBackground = useCallback(async () => {
    ...
    if (!isModelLoaded) {
      onError('Model not loaded yet. Please wait...');
      return;
    }
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <span>
          {isLoading ? 'Removing Background...' :
            !isModelLoaded ? 'Loading Model...' : 'Remove Background'}
        </span>
      </button>
