...
const FilterControls: React.FC<FilterControlsProps> = ({
  ...
  // Check if canvas has valid content for OpenCV
  const isCanvasValidForOpenCV = useCallback(() => {
    ...
    try {
      const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
      // Check if we have non-transparent content
      const hasContent = imageData.data.some((value, index) =>
        index % 4 !== 3 && value !== 0 // Check RGB channels only, ignore alpha
      );
      return hasContent && canvas.width > 0 && canvas.height > 0;
    } catch (error) {
      return false;
    }
  }, [canvas]);

  // OpenCV-based filters with better error handling
  const applyOpenCVFilter = useCallback(async () => {
    if (!canvas || !window.cv) {
      throw new Error('OpenCV.js not available');
    }

    if (!isCanvasValidForOpenCV()) {
      throw new Error('Canvas does not contain valid image data');
    }

    let src, dst, gray, sobelX, sobelY, kernel;

    try {
      console.log('Applying OpenCV filter:', filterType, 'on canvas size:', canvas.width, 'x', canvas.height);

      // Create a temporary canvas to work with (avoids CORS issues)
      ...
      switch (filterType) {
        case 'gaussian':
          const kernelSize = Math.max(3, Math.min(21, intensity * 2 + 1)); // Ensure odd number between 3-21
          const ksize = new window.cv.Size(kernelSize, kernelSize);
          window.cv.GaussianBlur(src, dst, ksize, intensity, intensity, window.cv.BORDER_DEFAULT);
          break;
        ...
        case 'sobel':
          ...
          sobelX = new window.cv.Mat();
          sobelY = new window.cv.Mat();

          // Convert to grayscale first
          window.cv.cvtColor(src, gray, window.cv.COLOR_RGBA2GRAY);

          // Apply Sobel with safe parameters
          const scale = Math.max(0.1, Math.min(5, intensity / 5));
          const delta = 0;
          const ddepth = window.cv.CV_16S; // Use 16-bit to avoid overflow

          window.cv.Sobel(gray, sobelX, ddepth, 1, 0, 3, scale, delta, window.cv.BORDER_DEFAULT);
          window.cv.Sobel(gray, sobelY, ddepth, 0, 1, 3, scale, delta, window.cv.BORDER_DEFAULT);

          // Convert back to 8-bit and combine
          const absX = new window.cv.Mat();
          const absY = new window.cv.Mat();
          window.cv.convertScaleAbs(sobelX, absX);
          window.cv.convertScaleAbs(sobelY, absY);
          window.cv.addWeighted(absX, 0.5, absY, 0.5, 0, dst);
        ...
        case 'sharpen':
          kernel = new window.cv.Mat(3, 3, window.cv.CV_32F);
          const data = new Float32Array([
            -1, -1, -1,
            -1, 9, -1,
            -1, -1, -1
        ...
        default:
          throw new Error(`Unsupported OpenCV filter: ${filterType}`);
      }

      // Validate result before displaying
      if (dst.empty()) {
        throw new Error('Filter result is empty');
      }
      ...
      console.error('OpenCV filter error details:', error);
      throw new Error(`OpenCV ${filterType} failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      // Clean up all matrices
      [src, dst, gray, sobelX, sobelY, kernel].forEach(mat => {
        if (mat && !mat.isDeleted) mat.delete();
      });
    }
  }, [canvas, filterType, intensity, isCanvasValidForOpenCV]);

  // Canvas2D-based filters (unchanged)
  const applyCanvasFilter = useCallback(() => {
    if (!canvas || !originalImageData) return false;
  ...
  }, [canvas, originalImageData, filterType, intensity]);

  // Apply filter
  const applyFilter = useCallback(async () => {
    if (!canvas || !originalImageData || isProcessingRef.current) {
      onError(!canvas ? 'No canvas available' : 'No image data available');
      return;
    }
    ...
    try {
      ...
      console.error('Filter processing failed:', error);
      onError(error instanceof Error ? error.message : 'Filter application failed');
    } finally {
      onProcessing(false);
      isProcessingRef.current = false;
    }
